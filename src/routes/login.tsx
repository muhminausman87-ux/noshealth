...
import { Activity, Lock, Phone, ShieldCheck, Stethoscope, User, AlertCircle, ArrowLeftRight } from "lucide-react";
...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Staff and admin sign-in for SyncCare EHR." },
    ],
  }),
  component: LoginPage,
});
...
function LoginPage() {
  ...
  const demoHints = useMemo(
    () =>
      role === "admin"
        ? [{ u: "admin", p: "admin123" }]
        : STAFF.filter((s) => s.role === "staff").slice(0, 4).map((s) => ({
            u: s.username,
            p: s.password,
          })),
    [role],
      ...
      <div className="mx-auto flex max-w-5xl flex-col items-center gap-6">
        ...
        <div className="grid w-full gap-6 md:grid-cols-[1fr_360px]">
          ...
          <div className="rounded-2xl border border-border bg-card p-6 shadow-sm md:p-8">
            <div className="mb-5 flex items-center gap-1 rounded-lg bg-secondary p-1">
              <RoleTab active={role === "staff"} onClick={() => setRole("staff")} icon={Stethoscope} label="Staff" />
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <Field icon={User} label="Username">
                <input
                  ...
                  placeholder={role === "admin" ? "admin" : "achen"}
                  ...
                  <label className="flex items-start gap-2 text-sm">
                    ...
                    <span>
                      <span className="flex items-center gap-1.5 font-medium text-foreground">
                        ...
                        Sign in as a pulled-out staff
                      </span>
                      <span className="text-xs text-muted-foreground">
                        Use this when you're covering a different unit than your home department.
                      </span>
                    </span>
                  </label>
                  {pulled && (
                    <div className="mt-3">
                      ...
                      <select
                        value={pulledDept}
                        onChange={(e) => setPulledDept(e.target.value as typeof pulledDept)}
            ...
            <div className="mt-5 border-t border-border pt-4">
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold">Call IT Support</span>
                </span>
          ...
          <aside className="rounded-2xl border border-border bg-card/70 p-5 text-sm shadow-sm">
            ...
            <p className="mb-3 text-xs text-muted-foreground">
              Prototype only — tap any account to autofill.
            </p>
            ...
            {role === "staff" && (
              <div className="mt-3 text-[11px] text-muted-foreground">
                Password for all staff: <span className="font-mono">nurse123</span>
              </div>
            )}
          </aside>
        </div>
